...
export async function login(email: string, password: string) {
    return await axios.post('http://localhost:8000/user/api/login/', {'email': email, 'password': password}, 
...
const LoginPage = () => {
    ...
    function handleSubmit(e) {
        e.preventDefault();
        console.log(inputs);
        login(inputs.email, inputs.password).then((response: AxiosResponse) => {
            if(response.status == 200){
                ...
                if(localStorage.getItem("token") != null) {
                    axios.get('http://localhost:8000/user/api/', {headers: {'Content-Type': 'application/json'}, withCredentials: true}).then(data => {
                        router.push("/");
                    });
                }
            }
